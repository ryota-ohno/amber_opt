...
import pandas as pd
import time
from make_8_xyz_ import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['theta','A2','a','b','z','E','E1','E2','E3','status'])##いじる
    ...
    if not os.path.exists(auto_csv_path3):        
        df_E_3 = pd.DataFrame(columns = ['theta','A2','a','b','z','E3','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    mono_file1=f'/home/HasegawaLab/ohno_amber/amber_opt/{monomer_name}/monomer/{monomer_name}_mono1.out'
    E_mono1=get_E(mono_file1)[0]
    mono_file2=f'/home/HasegawaLab/ohno_amber/amber_opt/{monomer_name}/monomer/{monomer_name}_mono2.out'
    E_mono2=get_E(mono_file2)[0]
    ...
    for idx, row in df_prg_1.iterrows():
        ...
        if len(E_list1)!=1 :##get Eの長さは計算した分子の数
            ...
            len_prg_1-=1
            E1=float(E_list1[0])-2*E_mono1##8分子に向けてep1,ep2作成　ep1:b ep2:a
    ...
    for idx, row in df_prg_2.iterrows():
        ...
        if len(E_list2) != 1:
            ...
            len_prg_2 -= 1
            E2 = float(E_list2[0]) -2*E_mono1  # Updated to E2
    ...
    for idx, row in df_prg_3.iterrows():
        ...
        if len(E_list3) != 1:
            ...
            len_prg_3 -= 1
            E3 = float(E_list3[0]) -2*E_mono2 # Updated to E3
